...
class UserTest(unittest.TestCase):
    def setUp(self):
        ...
        db_hose = app.config['DB_HOST']
        self.db_uri = "mysql+pymysql://%s:%s@%s/" % (db_username, db_password, db_host)
        app.config['TESTING'] = True
        app.config['WTF_CSRF_ENABLED'] = False
        app.config['BLOG_DATABASE_NAME'] = 'test_blog'
        app.config['SQLALCHEMY_URI'] = self.db_uri + app.config['BLOG_DATABASE_NAME']
        engine = sqlalchemy.create_engine(self.db_uri)
        conn = engine.connect()
        conn.execute('commit')
        conn.execute('CREATE_DATABASE ' + app.config['BLOG_DATABASE_NAME'])
        db.create_all()
        conn.close()
    ...
    def test_login_logout(self):
        self.create_blog()
        rv = self.login('avandelay', 'test')
        assert 'User avandelay logged in' in str(rv.data)
        rv = self.logout()
        assert 'User logged out' in str(rv.data)
        rv = self.login('avandelay', 'wrong password')
